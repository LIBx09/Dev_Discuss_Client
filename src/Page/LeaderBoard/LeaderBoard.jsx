        ...
        {users?.length === 0 ? (
          ...
          <div className="space-y-4">
            {users.map((user) => (
              <div
                key={user._id}
                className="rounded-xl p-4 shadow-lg border-l-4 border-blue-500  flex flex-col md:flex-row md:items-center justify-between z-10 gap-4 hover:scale-[1.01] transition-transform"
              >
                <div className="flex items-center gap-4">
                  ...
                  />
                  <div>
                    <p className="font-semibold text-blue-400">{user?.userName || "Not Found"}</p>
                    <p className="text-sm text-gray-400">{user?.userEmail || "Not Found"}</p>
                  </div>
                </div>
                <div className="text-left md:text-right space-y-1">
                  <p className="text-sm text-gray-300">Role: <span className="text-blue-400">{user?.role || "Not Found"}</span></p>
                  <p className="text-sm text-gray-300">Membership: <span className="text-blue-400">{user?.member || "Not Found"}</span></p>
                </div>
                <div className="dropdown dropdown-end">
                  <div tabIndex={0} role="button" className="btn btn-sm btn-outline text-blue-400 border-blue-400 hover:bg-blue-500 hover:text-white">
                    <BsThreeDots />
                  </div>
                  <ul tabIndex={0} className="dropdown-content menu rounded-box z-50 w-52 p-2 bg-white/10 backdrop-blur-md shadow-md border border-purple-500/20 text-sm">
                    <li>
