...
const ProblemSolve = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const { data } = await axios.get(`https://dev-discuss-server-kappa.vercel.app/problem/${id}`);
      setProblem(data);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const {data} = await axios.post(`https://dev-discuss-server-kappa.vercel.app/problemProgress/${id}`, {
         userCode,
        problemDes,
        email
      });
