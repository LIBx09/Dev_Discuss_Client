...
const Badges = ({ totalPoints = 0 }) => {
    ...
    return (
        <div>
            <h2 className="font-semibold bg-gradient-to-r from-blue-500 to-blue-700 text-transparent bg-clip-text text-center text-3xl pb-8">Badges</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-4">
                ...
                {badges.map((badge, index) => (
                    <div
                        ...
                        ${badge.unlocked ? "dark:bg-slate-800 border-blue-500 hover:shadow-lg" : "dark:bg-slate-600 opacity-70 border-blue-300 cursor-not-allowed"}`}
                    >
                        {!badge.unlocked && (
                            <>
                                ...
                                <div className="absolute top-0 left-0 w-full h-full rounded-xl bg-black bg-opacity-30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                    <span className="text-sm text-blue-400 bg-black/60 px-3 py-1 rounded-md">
                        ...
                        <h3 className="text-xl bg-gradient-to-r from-blue-500 to-blue-700 text-transparent bg-clip-text font-semibold mt-2 dark:text-white">{badge.badgeTitle}</h3>
                        <p className="text-sm dark:text-gray-300">{badge.BadegeDescription}</p>
                        <p className="mt-2 text-sm text-blue-400 dark:text-gray-300">Required: {badge.pointsRequired} pts</p>

                        {badge.unlocked ? (
                            <span className="inline-block mt-2 text-blue-400 font-semibold animate-pulse">
                                🏆 Unlocked!
                            </span>
                        ) : (
                            <span className="inline-block mt-2 text-blue-500 dark:text-gray-300 font-semibold">
                                🔒 Locked
                            </span>
                        )}
                    </div>
